...
df_final_4genes <- df_final_4genes %>%
  filter(source != "") %>%
  distinct() %>%
  group_by(source, cluster_type) %>%
...
df_final_4genes$num_cluster_types <- as.factor(df_final_4genes$num_cluster_types)

plot <- ggplot(df_final_4genes, aes(x=range_length)) +
  theme_gdocs() +
  geom_point(aes(y = num_clusters, colour = cluster_type),
             stat = "identity", fill = "lightgrey", size = 3, width = 0.1) +
