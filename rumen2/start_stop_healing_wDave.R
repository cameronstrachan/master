...
files <- list.files('~/master/rumen2/dataflow/03-analysis/mags/', pattern = '^start_stop')
list_dfs <- list()
i <- 1

for (file in files){
  df <- read.csv(paste('~/master/rumen2/dataflow/03-analysis/mags/', file, sep = ''), header=TRUE)
  df$file <- file
  list_dfs[[i]] <- df
  i <- i + 1
}
...
df$file <- gsub("start_stop_stewart2019_mags_genes_300_dave", "", df$file)
...
write.csv(df_start_stop_clean, '~/master/rumen2/dataflow/03-analysis/compiled_start_stop_wDave.txt')
