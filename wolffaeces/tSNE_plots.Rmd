...
df_phylocounts_meta <- read.csv("~/master/wolffaeces/dataflow/00-meta/sample-metadata.csv")
...
df_phylocounts_meta <- df_phylocounts_meta[-111, ]

df_phylocounts_meta <- df_phylocounts_meta[grepl("f$", df_phylocounts_meta$SampleID),]
...
df_phlycounts_counts <- read.csv("~/master/wolffaeces/dataflow/03-asv-table/feature-table-decontam.txt", sep = '\t', skip = 1)

df_phlycounts_counts <- cbind(df_phlycounts_counts$X.OTU.ID, df_phlycounts_counts[,grepl("f$", names(df_phlycounts_counts))])
...
#tree = read.tree("~/master/wolffaeces/dataflow/03-asv-seqs/tree-decontam.nwk")
