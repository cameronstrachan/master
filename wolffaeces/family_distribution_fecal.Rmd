...
df_counts_phylum <- df_counts %>%
  ...
  group_by(samples, family) %>% 
  mutate(norm_family = sum(norm)) %>%
  ungroup() %>%
  select(samples, family, norm_family) %>%
  distinct() 
...
df_top_phyla <- df_counts_phylum %>%
  group_by(family) %>%
  mutate(sum_norm = sum(norm_family)) %>%
  select(family, sum_norm) %>%
  distinct() %>%
  filter(sum_norm > 0.2)
  
df_top_phyla_vec <- unique(df_top_phyla$family)

df_counts_phylum$family <- as.character(df_counts_phylum$family)

df_counts_phylum <- df_counts_phylum %>%
  mutate(family_clean = if_else(family %in% df_top_phyla_vec, family, 'Other'))
...
plevels <- c("Peptostreptococcaceae", "Lachnospiraceae" , "Erysipelotrichaceae", "Clostridiaceae 1", "Staphylococcaceae", "Lactobacillaceae", "Ruminococcaceae", "Streptococcaceae", "Enterococcaceae",  "Veillonellaceae", "Planococcaceae", "Acidaminococcaceae", "Clostridiales_Incertae Sedis XI", "Not Available")
df_counts_phylum$family_clean <- as.character(df_counts_phylum$family_clean)
df_counts_phylum$family_clean[is.na(df_counts_phylum$family_clean)] <- "Not Available"
df_counts_phylum$family_clean <- factor(df_counts_phylum$family_clean, levels = plevels)

plot <- ggplot() + geom_bar(
  aes(y = norm_family, x = samples, fill = family_clean, order = norm_family),
