...
for (x in 1:length(files)) {
  ...
  df_reads <- df_trim %>%
    ...
    unite(header, c("sample", "observation"), sep = "_")
...
for (j in 1:nrow(df_compiled_reads)) {
    reads_list[[k]] <- paste(">", df_compiled_reads[j, 1], sep = "")
    k <- k + 1

    reads_list[[k]] <- df_compiled_reads[j, 2]
    k <- k + 1
}

reads_matrix <- as.matrix(unlist(rbind(reads_list)))
reads_matrix <- gsub("\"", "", reads_matrix)
write.table(reads_matrix, '~/master/chyo/mapped_reads_trimmed.fasta', row.names = FALSE, 
