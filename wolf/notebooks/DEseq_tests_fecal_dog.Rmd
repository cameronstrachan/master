...
df <- df %>%
  select(asv_id, starts_with("D")) %>%
  select(asv_id, ends_with("f"))

col_to_gather <- names(df)[!(startsWith(names(df), "D"))]
...
#df_plot$genus <- gsub("Corynebacterium 1", "Corynebacterium", df_plot$genus)
df_plot$pack <- gsub("D_pack_", "", df_plot$pack)

df_plot$genus[is.na(df_plot$genus)] <- "Enterobacteriaceae"
