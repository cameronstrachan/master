...
# 
# physeq_df <- as.data.frame(physeq_norm@otu_table)
# physeq_df$asv_id <- rownames(physeq_df)
# rownames(physeq_df) <- c()
# 
# df.pres$asv_id <- rownames(df.pres)
# 
# df_contamination_labelled <- df.pres %>%
#   select(asv_id, prevalence.neg)
# 
# rownames(contamination_labelled) <- c()
# 
# df_removed <- as.data.frame(my_contaminants)
# colnames(df_removed) <- "asv_id"
# df_removed$removed <- TRUE
# 
# df_contamination_assesment <- inner_join(physeq_df, df_contamination_labelled) %>%
#   full_join(df_removed)
# 
# df_contamination_assesment[c("removed")][is.na(df_contamination_assesment[c("removed")])] <- FALSE
# 
# df_contamination_assesment_not_removed <- df_contamination_assesment %>%
#   rowwise() %>%
#   mutate(not_removed_present = ifelse(removed == FALSE & prevalence.neg > 0, TRUE, FALSE)) %>%
#   filter(not_removed_present  == TRUE)
