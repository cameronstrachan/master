...
inputFile="~/master/anna/dataflow/01-nucl/n_stinkeri.fna"
...
df_tet_n_stinki <- as.data.frame(oligoFreqCorMatrix)
df_tet_n_stinki$x_axis <- apply(oligoFreqCorMatrix[,5:6],1,mean)
df_tet_n_stinki$y_axis <- oligoFreqCorMatrix[,10]
```

```{r echo=FALSE, message=FALSE, warning=FALSE}
df_stinki_headers <- read.csv("~/master/anna/dataflow/02-headers/n_stinkeri_rename.fna.csv")

df_stinki_headers <- df_stinki_headers %>%
  separate(X0, into = c("rm", "start", "stop", "strand"), sep = " # ") 

df_stinki_headers$stop <- as.numeric(df_stinki_headers$stop)
df_stinki_headers$start <- as.numeric(df_stinki_headers$start)
...
colnames(df_stinki_headers)[1] <- "qseqid"

df_stinki_headers$qseqid <- gsub("n_", "", df_stinki_headers$qseqid)

df_stinki_headers <- df_stinki_headers[startsWith(df_stinki_headers$qseqid, "stinkeri_1"), ]

df_stinki_AvA <- inner_join(df_plot_stinki, df_stinki_headers) %>%
  select(orf_center, mean_pi, genome2) 

colnames(df_stinki_AvA)[1] <- "x_axis"

df_tet_n_stinki <- df_tet_n_stinki %>%
  filter(scaffoldLength == 3309797) %>%
  select(x_axis, y_axis)

df_combined_stinki <- bind_rows(df_tet_n_stinki, df_stinki_AvA)
```

```{r echo=FALSE, message=FALSE, warning=FALSE, fig.width=20, fig.height=5}
library(cowplot)

p1 <- ggplot(df_combined_stinki, aes(x_axis)) +
...
p2 <- ggplot(df_combined_stinki, aes(x_axis, y_axis)) + 
  geom_line(data = df_combined_stinki[!is.na(df_combined_stinki$y_axis), ], size = 0.5, color = "grey43") + 
