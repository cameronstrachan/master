...
inputFile="~/master/anna/dataflow/01-nucl/n_noahi.fna"
...
df_tet_n_noahi <- as.data.frame(oligoFreqCorMatrix)
df_tet_n_noahi$x_axis <- apply(oligoFreqCorMatrix[,5:6],1,mean)
df_tet_n_noahi$y_axis <- oligoFreqCorMatrix[,10]
```

```{r echo=FALSE, message=FALSE, warning=FALSE}
df_noahi_headers <- read.csv("~/master/anna/dataflow/02-headers/n_noahi_rename.fna.csv")

df_noahi_headers <- df_noahi_headers %>%
  separate(X0, into = c("rm", "start", "stop", "strand"), sep = " # ") 

df_noahi_headers$stop <- as.numeric(df_noahi_headers$stop)
df_noahi_headers$start <- as.numeric(df_noahi_headers$start)
...
colnames(df_noahi_headers)[1] <- "qseqid"

df_noahi_headers$qseqid <- gsub("n_", "", df_noahi_headers$qseqid)

df_noahi_headers <- df_noahi_headers[startsWith(df_noahi_headers$qseqid, "noahi_1"), ]

df_noahi_AvA <- inner_join(df_plot_noah, df_noahi_headers) %>%
  select(orf_center, mean_pi, genome2) 

colnames(df_noahi_AvA)[1] <- "x_axis"

df_tet_n_noahi <- df_tet_n_noahi %>%
  filter(scaffoldLength == 3921641) %>%
  select(x_axis, y_axis)

df_combined_noahi <- bind_rows(df_tet_n_noahi, df_noahi_AvA)
```

```{r echo=FALSE, message=FALSE, warning=FALSE, fig.width=20, fig.height=5}
library(cowplot)

p1 <- ggplot(df_combined_noahi, aes(x_axis)) +
...
p2 <- ggplot(df_combined_noahi, aes(x_axis, y_axis)) + 
  geom_line(data = df_combined_noahi[!is.na(df_combined_noahi$y_axis), ], size = 0.5, color = "grey43") + 
