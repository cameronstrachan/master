...
seq <- readDNAStringSet("~/master/epithelial/dataflow/01-nucl/sanger_strain_library_oneLine.fasta")
...
df_final <- inner_join(df_data, df_taxa) %>%
  ...
  select(asv, Type, isolate, phylum, family, genus, per_samples_oberved, median_percent) %>% 
  distinct() %>%
  filter(per_samples_oberved > 10) %>%
  filter(median_percent > 0.01) %>%
