...
df_blast <- read.delim("~/master/strain_collection/02-blast/strains_to_wetzels_goat.txt", header=FALSE)
colnames(df_blast)[1:13] <- c("qseqid", "sseqid", "pident", "sstart", "send", "qstart", "qend", "evalue", "bitscore", "score", "qlen", "length", "sseq")
...
df_count <- read.delim("~/master/strain_collection/03-tables/wetzels_et_al_epithelial_goat.txt", skip = 1)
names(df_count)[1] <- 'sseqid'

#df_meta <- read.csv("~/master/strain_collection/00-meta/wetzels_epithelial_goat.csv")
#df_meta$sample <- paste("X", df_meta$ID, sep = "")
...
df_final_wetzels <- df_count %>%
  ...
  #inner_join(df_meta) %>%
  inner_join(df_classification) %>%
  select(sseqid, sample, count, total_reads, qseqid, pident, length, percent_sample, order, family, genus)
