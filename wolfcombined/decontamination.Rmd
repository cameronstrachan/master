...
df_phlycounts_counts <- read.csv("~/master/wolfcombined/dataflow/03-asv-table/feature-table.txt", sep = '\t', skip = 1)
...
df_negative_control <- df_phlycounts_counts %>%
  filter(NTCs > 0 | NTCf > 0) %>%
  mutate(total_negs = sum(NTCs)) %>%
  mutate(total_negf = sum(NTCf)) %>%
  gather(sample, count, -asv_id, -NTCs, -NTCf, -total_negs, -total_negf) %>%
  filter(count > 0) %>%
  group_by(asv_id) %>%
  mutate(prev = length(unique(sample))) %>%
  ungroup() %>%
  mutate(rel_negs = (NTCs/ total_negs)*100) %>%
  mutate(rel_negf = (NTCf/ total_negf)*100) %>%
  filter(rel_negs > 3) %>%
  filter(rel_negf > 3) 
...
df_physeq_decontam <- as.data.frame(OTU)
...
write.table(mat_final, '~/master/wolfcombined/dataflow/03-asv-table/feature-table-decontam.txt', col.names = FALSE, quote = FALSE, row.names = FALSE, sep = "\t")
```