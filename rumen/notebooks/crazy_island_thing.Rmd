...
df_agsinst_RUG782 <- island_mapping %>%
  select(qseqid, sseqid, pident, sstart, send, bitscore, length) %>%
...
df_seg_1 <- dna_seg(read.csv("~/master/rumen/dataflow/00-meta/4309680.csv")) 
df_seg_2 <- dna_seg(read.csv("~/master/rumen/dataflow/00-meta/CP022905.csv"))

dna_segs <- list(df_seg_1, df_seg_2)

names <- c("4309680", "CP022905")
names(dna_segs) <- names

comparison1 <- as.comparison(read.csv("~/master/rumen/dataflow/00-meta/2islands_comparison.csv"))
comparisons <- list(comparison1)

comparisons[[1]]$col <- apply_color_scheme(c(0.6, 0.4, 0.5, 0.6, 0.4, 0.5, 0.6, 0.4, 0.5, 0.6), "grey")

plot_gene_map(dna_segs=dna_segs, comparisons=comparisons, xlims=xlims, main="Comparison between 4309680 and CP022905")
