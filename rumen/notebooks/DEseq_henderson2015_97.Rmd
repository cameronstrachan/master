...
df_classification  <- read.csv("~/master/rumen/dataflow/03-asv-taxonomy/henderson2015-4_194-97-rdp.csv")
...
df <- read.delim("~/master/rumen/dataflow/03-asv-table/henderson2015-4_194-97.txt", skip = 1, header = TRUE)
df$clustering <- 97
...
colnames(df_direction) <- "fold_change"
df_direction$asv_id <- selected_seq_ids
df_direction <- df_direction %>%
  rowwise() %>%
  mutate(direction = ifelse(fold_change > 0, "pos_cor", "neg_cor"))

df_plot <- 
df_plot %>% filter(genus == "Prevotella") %>%
