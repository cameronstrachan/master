...
# group by the category opf resistance and then make sure there is more than 1, plot there genes
...
resistances$gene <- gsub("Agrobacterium fabrum chloramphenicol acetyltransferase", "cat", resistances$gene)
...
resistances_summary$file <- gsub("_rename", "", resistances_summary$file)
resistances_summary$file <- gsub("submission.assembly", "", resistances_summary$file)
resistances_summary$file <- gsub("final.assembly", "", resistances_summary$file)
resistances_summary$file <- gsub(".assembly", "", resistances_summary$file)
resistances_summary$file <- gsub("-", "", resistances_summary$file)
resistances_summary$file <- gsub("_IMGtaxon_2693429877_annotated", "", resistances_summary$file)
resistances_summary$file <- gsub("_genomic", "", resistances_summary$file)
resistances_summary$file <- gsub("\\.", "", resistances_summary$file)

df_all_files <- df_pi_filled %>%
  select(file1) %>%
  distinct() %>%
  rename(file = file1)
  
resistances_summary <- full_join(resistances_summary, df_all_files)

check <- inner_join(df_meta, resistances)

resistances_summary <- resistances_summary %>%
  gather(resistance, presence, -file)

resistances_summary$presence[!(is.na(resistances_summary$presence))]<-1
resistances_summary$presence[is.na(resistances_summary$presence)]<-0


resistances_summary <- resistances_summary[resistances_summary$file %in% order,]

resistances_summary$file <- factor(resistances_summary$file,
                           levels = order, 
                           ordered = TRUE)


resistance_plot <- ggplot(data = resistances_summary, aes(x = resistance, y = file)) +
...
#plot_grid(heatmap.plot, resistance_plot, align = 'h', ncol = 2)
#Virginiamycin
```
