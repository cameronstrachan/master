...
qiime phylogeny align-to-tree-mafft-fasttree \
  --i-sequences dataflow/02-qiime-control/rep-seqs-dn-97.qza \
  --o-alignment dataflow/02-qiime-control/aligned-rep-seqs-dn-97.qza \
  --o-masked-alignment dataflow/02-qiime-control/masked-aligned-rep-seqs-dn-97.qza \
  --o-tree dataflow/02-qiime-control/unrooted-tree.qza \
  --o-rooted-tree dataflow/02-qiime-control/rooted-tree.qza

qiime feature-table summarize \
  --i-table dataflow/02-qiime-control/table-dn-97.qza \
  --o-visualization dataflow/02-qiime-viz-control/table-dn-97.qzv \
  --m-sample-metadata-file dataflow/00-meta-control/sample-metadata.tsv
...
qiime diversity alpha-rarefaction \
  --i-table dataflow/02-qiime-control/table-dn-97.qza \
  --i-phylogeny dataflow/02-qiime-control/rooted-tree.qza \
  --p-max-depth $sample_depth \
  --m-metadata-file dataflow/00-meta-control/sample-metadata.tsv \
  --o-visualization dataflow/02-qiime-viz-control/alpha-rarefaction.qzv

qiime taxa barplot \
  --i-table dataflow/02-qiime-control/table-dn-97.qza \
  --i-taxonomy dataflow/02-qiime-control/silva-taxonomy.qza \
  --m-metadata-file dataflow/00-meta-control/sample-metadata.tsv \
  --o-visualization dataflow/02-qiime-viz-control/taxa-bar-plots.qzv
