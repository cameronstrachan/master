...
qiime tools import \
  ...
  --input-path dataflow/01-fastq-control/trimmed/ \
  --input-format CasavaOneEightSingleLanePerSampleDirFmt \
  --output-path dataflow/02-qiime-control/demux-trimmed.qza
  
qiime demux summarize \
  --i-data dataflow/02-qiime-control/demux-trimmed.qza \
  --o-visualization dataflow/02-qiime-viz-control/demux-trimmed.qzv
